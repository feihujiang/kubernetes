...
docker run ${user_flags} --rm -v $V1_TMP_IN_HOST:/output:rw -v ${SWAGGER_PATH}:/swagger-source:z gcr.io/google_containers/gen-swagger-docs:v4.2 \
    v1 \
    https://raw.githubusercontent.com/kubernetes/kubernetes/master/pkg/api/v1/register.go

docker run ${user_flags} --rm -v $V1BETA1_TMP_IN_HOST:/output:rw -v ${SWAGGER_PATH}:/swagger-source:z gcr.io/google_containers/gen-swagger-docs:v4.2 \
    v1beta1 \
